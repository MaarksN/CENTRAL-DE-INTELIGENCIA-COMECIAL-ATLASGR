...
const manifest = (competitionStatus: 'ATUALIZADO' | 'PARCIAL'): MarketIntelligenceManifest => ({
    schemaVersion: '1.0',
...
describe('Market Intelligence final decision readiness', () => {
    it('mantém decisão final bloqueada quando concorrência é parcial e economics estão incompletos', async () => {
        mockFetch(manifest('PARCIAL'), [territory()]);
        ...
        expect(snapshot.manifest.decisionBlockers.join(' ')).toContain('concorrência');
        expect(snapshot.manifest.decisionBlockers.join(' ')).toContain('SAM');
        expect(snapshot.manifest.decisionBlockers.join(' ')).toContain('MRR');
        expect(snapshot.manifest.decisionBlockers.join(' ')).toContain('Break-even');
    });

    it('preserva decisionReady quando os finalistas têm censo completo e economics necessários', async () => {
        mockFetch(manifest('ATUALIZADO'), [territory({
