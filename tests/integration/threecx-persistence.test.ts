...
const asOrg = <T>(organizationId: string, fn: () => Promise<T>): Promise<T> =>
    runInContext({ tenantId: organizationId }, fn);
...
describe('Persistência de ThreeCXConnection contra Postgres real', () => {
    ...
    it('cifra apiKey/apiSecret em repouso — a linha crua no banco não contém o segredo em texto puro', async () => {
        ...
        // está fisicamente gravado na coluna. $queryRaw/$executeRaw não passam por $allOperations
        // (só intercepta operações de model), então precisam de withRlsContext explícito — não
        // basta AsyncLocalStorage estar correto, o helper injeta o set_config na MESMA transação
        // interativa da query crua (ver src/lib/prisma.ts).
