...
SCRIPT = ROOT / "public/tools/atlas-market-intelligence/etl_concorrencia_censo.py"
spec = importlib.util.spec_from_file_location("etl_concorrencia_censo", SCRIPT)
...
class CompetitionCensusEtlTest(unittest.TestCase):
    def setUp(self):
        ...
        self.municipios = [
            {"ibgeCode": "3543402", "name": "Ribeirão Preto", "uf": "SP", "region": "Sudeste", "latitude": -21.17, "longitude": -47.81},
            {"ibgeCode": "3550308", "name": "São Paulo", "uf": "SP", "region": "Sudeste", "latitude": -23.55, "longitude": -46.63},
        ]
        (self.base / "municipios.json").write_text(json.dumps(self.municipios, ensure_ascii=False), encoding="utf-8")
    ...
    def test_complete_requires_every_protocol_check(self):
        _, by_name = module.geography_indexes(self.municipios)
        by_code, _ = module.geography_indexes(self.municipios)
        coverage = self.write_csv(
            "coverage.csv",
            [
                "ibge_code", "municipio", "uf", "protocol_version",
                *module.REQUIRED_CHECKS, "reviewed_by", "verified_at", "confidence", "census_status",
            ],
            [{
                "ibge_code": "3543402",
                ...
                "verified_at": "2026-08-22T12:00:00-03:00",
                "confidence": "ALTO",
                "census_status": "CENSO_COMPLETO",
            }],
        )
        metrics = {}
        _, requested, accepted = module.apply_coverage(metrics, coverage, by_code, by_name)
    ...
    def test_complete_is_accepted_only_with_full_protocol(self):
        by_code, by_name = module.geography_indexes(self.municipios)
        row = {
            "ibge_code": "3550308",
            "municipio": "São Paulo",
            "uf": "SP",
        ...
        for field in module.REQUIRED_CHECKS:
            row[field] = "sim"
        coverage = self.write_csv(
            "coverage.csv",
            [
    ...
    def test_presence_never_implies_complete_census(self):
        ...
        metrics, unmatched = module.load_presences(presences, by_code, by_name)
